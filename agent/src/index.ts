    ...
    if (plugins.length > 0) {
        ...
        const importedPlugins = await Promise.all(
            plugins.map(async (plugin) => {
                try {
                    if (plugin === "@elizaos/plugin-forum-analyzer") {
                        return new ForumAnalyzerPlugin({
                            platforms: {
                                discourse: {
                                    usePublicDiscourse: true,
                                    baseUrl: "https://forum.decentraland.org"
                                }
                            }
                        });
                    }
        ...
        return importedPlugins.filter(p => p !== null);
    }
    return [];
}
...
// Comment out problematic database adapter initialization
...
// Comment out problematic client initializations
